...
## Phase 4: FastAPI Application Layer [COMPLETED]
- [x] **Task 4.1: FastAPI Scaffold, uvloop Engine & Lifespan Pool Management**
...
- [x] **Task 4.2: Zero-Network Supabase JWT Auth & Claims LRU Cache**
...
- [x] **Task 4.3: Food Vision Analysis Endpoint with Compression Guard (`POST /api/v1/meals/analyze`)**
...
- [x] **Task 4.4: Single-Query CTE Atomic Persistence Endpoint (`POST /api/v1/meals/confirm`)**
...
- [x] **Task 4.5: Instant-Flush SSE Conversational Agent Endpoint (`POST /api/v1/chat`)**
...
- [x] **Task 4.6: Sub-Millisecond GIN-Indexed Daily Dashboard Endpoint (`GET /api/v1/dashboard/daily`)**
...
- [x] **Task 4.7: User Profile & Goals Management Endpoints (`GET/PUT /api/v1/profile`)**
  - `GET /api/v1/profile`: Retrieve user profile, BMR, TDEE, macro targets, and long-term memory facts.
  - `PUT /api/v1/profile`: Update user biometric profile, calculate BMR/TDEE via Mifflin-St Jeor, and UPSERT calorie targets and macro ratios.

- [x] **Task 4.8: Middleware Stack, Security Headers & Rate Limiter**
  - Configure OWASP production security headers (`SecurityHeadersMiddleware`).
  - Add request timing header `X-Process-Time` (`ProcessTimeMiddleware`).
  - Add in-memory sliding window rate limiter (`RateLimiterMiddleware`).

- [x] **Task 4.9: Automated Zero-Latency FastAPI & Integration Test Suite**
  - `tests/test_api_auth.py`: In-memory JWT authentication & claims cache testing.
  - `tests/test_api_meals.py`: Food vision `/analyze` & single-query CTE `/confirm` endpoints.
  - `tests/test_api_chat.py`: `/chat` instant-flush SSE streaming and status events.
  - `tests/test_api_dashboard.py`: `/dashboard/daily` macro budget and timeline cards.
  - `tests/test_api_profile.py`: Profile GET and PUT endpoints + BMR/TDEE calculations.
  - `tests/test_api_middleware.py`: Security headers, timing metrics, and rate limiting.
  - `tests/test_phase_4_e2e_integration.py`: Complete user journey E2E integration test suite.
